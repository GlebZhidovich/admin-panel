...
export function PunktEdit() {
	...
	return (
		<div className={classes.root}>
			<Title>Пункт ТО - Тверь, ул. Ленина 1</Title>
			{punktFields.map(field => (
					...
					<List className="flex">
						<div>
							...
							{[
								...
								<ListItem key={item.title} className="flex justify-between">
									<FormControlLabel
										value="end"
			...
			<div className="flex min-w-0">
				...
				<div className="flex flex-wrap pl-16">
					{new Array(16).fill('Адыгея').map((title, i) => (
						<FormControlLabel
							key={i}
							value="end"
							control={<Checkbox color="primary" checked />}
			...
			<Title>Эксперты, работающие на этом пункте</Title>
			<List className="p-16">
				{expertsList.map((title, i) => (
					<ListItem key={i}>
						<Typography variant="body1">{`${i + 1}. ${title}`}</Typography>
					</ListItem>
				))}
			</List>

			<Title>Фотографии фона пункта ТО</Title>
			<div className="flex">
			...
			<div>
				<Button className="p-16 text-white" color="secondary" variant="contained">
					Сохранить
				</Button>
			</div>
		</div>
